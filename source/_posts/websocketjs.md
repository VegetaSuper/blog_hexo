...
categories:
  ...
  - WebSocket
  - 长连接
date: 2022-05-09 16:16:05
cover: http://rblypd9jp.bkt.clouddn.com/picture/webscoket.png
---
...
```javascript
export default function longSock(url, fn, intro = '') {
...
```javascript
<script>
import longSock from '@/utils/ws.js'
export default {
  data() {
    return {
      ws: null
  ...
  methods: {
    //初始化weosocket
    initWebSocket() {
      const url = '127.0.0.1' // 后端的地址
      this.ws = longSock(`ws://${url}`, this.websocketonmessage, '连接ws=>')
    },
    // 数据接收
